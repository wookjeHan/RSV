...
def main(args):
    ...
    shot_selector_func = getattr(shot_selectors, args.shot_selector)

    print(f"Evaluating the performace of {args.shot_num} shot {args.shot_selector}")
    if shot_selector_func == shot_selectors.ours:
        ...
        for repeat in range(4):
            ...
            val_acc = test(language_model, tokenizer, val_dataloader, shot_selector, truncator)
            test_acc = test(language_model, tokenizer, test_dataloader, shot_selector, truncator)
            print(f"[{repeat}/4] Val Acc {val_acc * 100:.1f} Test Accuracy {test_acc * 100:.1f}")
            del shot_selector
...
if __name__ == '__main__':
    ...
    parser.add_argument('--shot_selector', type=str, default='ours')
